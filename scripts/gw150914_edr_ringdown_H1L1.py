# scripts/gw150914_edr_ringdown_H1L1.py
"""
Comparación de modelos de ringdown para GW150914 usando la combinación H1+L1:
...
Se combinan las señales de H1 y L1 en la ventana de ringdown como:
  h_comb = (h_H1 + h_L1)/2

y se ajustan los modelos a h_comb_normalizada.

Requiere que los archivos HDF5 de H1 y L1 estén en data/,
generados por scripts/download_data.py
"""
...
def chi2_stats(y: np.ndarray, y_model: np.ndarray, n_params: int) -> tuple[float, float, float, float]:
    """Devuelve S, chi2_red, AIC y BIC (sigma = 1)."""
    n = len(y)
    resid = y - y_model
    S = np.sum(resid**2)
    chi2 = S
...
def main() -> None:
    ...
    if not os.path.exists(h1_file) or not os.path.exists(l1_file):
        raise FileNotFoundError(
            "Faltan archivos H1 o L1 en data/. Ejecuta scripts/download_data.py"
        )

    # 1. Cargar señales H1 y L1
    t_H1, h_H1 = load_strain_from_hdf5(h1_file)
    t_L1, h_L1 = load_strain_from_hdf5(l1_file)

    # Asegurarse de que tienen la misma longitud
    n = min(len(t_H1), len(t_L1))
    t_H1 = t_H1[:n]
    h_H1 = h_H1[:n]
    t_L1 = t_L1[:n]
    h_L1 = h_L1[:n]
    ...
    # 2. Restar medias
    h_H1 = h_H1 - np.mean(h_H1)
    h_L1 = h_L1 - np.mean(h_L1)

    # 3. Encontrar picos y definir una ventana común centrada en el pico de H1
    idx_peak_H1 = np.argmax(np.abs(h_H1))
    t_peak_H1 = t_H1[idx_peak_H1]

    # Usamos la misma ventana que antes: 2 ms a 30 ms detrás del pico de H1
    t_start = t_peak_H1 + 0.002
    t_end   = t_peak_H1 + 0.030

    mask_H1 = (t_H1 >= t_start) & (t_H1 <= t_end)
    mask_L1 = (t_L1 >= t_start) & (t_L1 <= t_end)

    # Asegurar que las máscaras dan el mismo número de puntos
    idxs = np.where(mask_H1 & mask_L1)[0]
    t_win = t_H1[idxs]
    h_H1_win = h_H1[idxs]
    h_L1_win = h_L1[idxs]

    # 4. Señal combinada simple
    h_comb = 0.5 * (h_H1_win + h_L1_win)

    # 5. Definir tiempo relativo y normalizar
    t_rel = t_win - t_win[0]
    h_rms = np.sqrt(np.mean(h_comb**2))
    h_norm = h_comb / h_rms if h_rms != 0 else h_comb

    print(f"Ventana combinada H1+L1: duración ~ {t_rel[-1] - t_rel[0]:.6f} s, puntos = {len(t_rel)}")
    print(f"RMS de la ventana combinada: {h_rms:.3e}")
    ...
    print("\n[Ajuste Modelo 0 (H1+L1): GR (solo 220)]")
    ...
    print("\n[Ajuste Modelo 1 (H1+L1): GR + alpha_flow (220 corregido, sin X)]")
    ...
    print("\n[Ajuste Modelo 2 (H1+L1): GR + modo X (alpha_flow = 0)]")
    ...
    print("\n[Ajuste Modelo 3 (H1+L1): EDR completo (220 corregido + X)]")
    ...
    print("\n=== Comparación de modelos (H1+L1, ventana normalizada) ===")
    ...
    # 5. Figura datos combinados + M0 + M3 + residuales
    h0_phys = h0 * h_rms
    h3_phys = h3 * h_rms
    ...
    ax1.plot(t_rel * 1000, h_comb,    label="Datos H1+L1 (ringdown)", lw=1, color="C0")
    ...
    ax1.set_title("GW150914 (H1+L1): Datos vs GR y EDR")
    ax1.grid(True)
    ax1.legend()
    ...
    fig.tight_layout()
    out_png = os.path.join(repo_root, "data", "gw150914_H1L1_edr_ringdown.png")
